# Separate-and-Detect — Audio Demo Page

**🎵 Live Demo Page:** [https://ismir26snd.github.io/demoSnD/](https://ismir26snd.github.io/demoSnD/)

This demo page showcases our separation-and-transcription pipeline for automatic drum transcription (ADT). It contains:

- Abstract: a short overview of the task and method.
- Pianoroll + Six‑track Audios (Two examples):
  - For each example: one pianoroll image.
  - Four 6‑audio sets per example: Target, Prediction—ADTOF, Prediction—LarsNet, Prediction—MSG‑LD (+OB).
  - Each set includes Mixture + the five stems above.

### Data note
Some of the drum sounds in our examples are rendered from one-shot samples extracted from the StemGMD dataset.
